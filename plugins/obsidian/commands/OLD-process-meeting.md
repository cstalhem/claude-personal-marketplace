...
## Background context
1. Provide the raw transcript file to the process-script: `bash ${CLAUDE_PLUGIN_ROOT}/scripts/process-transcript-outline.sh [transcript-path]`
   - Never read transcript files directly, only read the output from the script
2. Check the meeting template, tagging conventions, and note-taking conventions

## Workflow
1. Rewrite the content from the transcript processing script to match the template according to our conventions
2. Capture action-items as tasks with clear owners and due-dates when possible
3. Update the note front matter and Tag the note
4. Separate on-topic discussion from off-topic discussion in the appropriate sections
